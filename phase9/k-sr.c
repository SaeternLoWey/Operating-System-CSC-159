...
int ForkSR(void)
{
    ...
    pcb[child_pid].main_table = pcb[run_pid].main_table;
...
void ExecSR(int code, int arg)
{
    ...
    // Panic if you can't find 5
    if(count != 5)
    {
        cons_printf("Panic: not enough pages!");
        return;
    }
     
    // Mark pages as used by run_pid and calculate their addresses
    for(y = 0; y < 5; y++)
    {
        page_user[pages[y]] = run_pid;
        pages[y] = (int)(pages[y] * PAGE_SIZE + RAM);
    }
    
    // Copy code to code page
    MemCpy((char *) pages[CODE_PAGE], (char *) code, PAGE_SIZE);

    // Clear stack page and put arg at the top
    Bzero((char *) pages[STACK_PAGE], PAGE_SIZE);
    p = (int *)(pages[STACK_PAGE] + PAGE_SIZE);
    p--;
    *p = arg;

    p--; // skip 4 bytes (not used)

    // Build trapframe in stack page
    q = (trapframe_t *) p;
    q--;
    ...
    pcb[run_pid].trapframe_p->eip = M256;

    // Build main table (for address translation)
...
void KillSR(int pid, int sig_num)
{
    ...
    if(pid == 0)
    {
        for(x = 0; x < PROC_SIZE; x++)
        {
            if(pcb[x].ppid == run_pid && pcb[x].state == PAUSE)
            {
                pcb[x].state = READY;
                EnQ(x, &ready_q);
            }
